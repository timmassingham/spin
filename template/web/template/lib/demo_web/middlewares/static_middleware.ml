...
let respond_with_file ?mime_type ?headers ~read name =
  ...
  | None ->
    ...
    let resp = Rock.Response.make ~headers ~status:`Not_found () in
    Lwt.return resp
  | Some body ->
    let mime_type =
      mime_type |> Option.value ~default:(Magic_mime.lookup name)
    in
    let headers =
...
let serve
    ~read ?mime_type ?etag_of_fname ?(headers = Httpaf.Headers.empty) fname req
  =
  let etag_quoted =
  ...
  if request_matches_etag then
    Lwt.return @@ Rock.Response.make ~status:`Not_modified ~headers ()
  else
    respond_with_file ~read ?mime_type ~headers fname

let m ~read ?(uri_prefix = "/") ?headers ?etag_of_fname () =
  let filter handler req =
    if req.Rock.Request.meth = `GET then
      ...
      if local_path |> is_prefix ~prefix:uri_prefix then
        ...
        match res.status with `Not_found -> handler req | _ -> Lwt.return res
      else
        handler req
    else
      handler req
